...
=======
            if (is_menu_visible) {
                ...
                //printf("wrap: %d text_size : %d Length of text : %d Size of title :%d Length of title Box: %d \n",wrap_title_index,text_size,TextLength(title),MeasureText(title, text_size),(int)title_box.width);
                DrawText(&title[wrap_title_index], title_box.x+5, title_pos.y, text_size, ACCENT_COLOR2);
                // }else{
                //     DrawText(title, title_box.x +
                //     5,title_pos.y,text_size,ACCENT_COLOR2);
                // }
                if (mouse_on_title) {
                    CLOCK_TIME now = get_current_clock_local();
                    if (title_letter_count < SCHEDULE_TITLE_MAX_LEN) {
                        if (now.second % 2 == 0) {
                            if (wrap_title_index > 0) {
                                printf("_ pos x on wrap : %d\n",(int)title_box.width);
                                DrawText("_", title_box.width + title_box.x - text_size , title_box.y + title_box.height-text_size, text_size, ACCENT_COLOR2);
                            } else {
                                printf("_ pos x : %d\n",(int) title_box.x + 10 + MeasureText(title, text_size));
                                DrawText("_",title_box.x + 10 + MeasureText(title, text_size),title_box.y + title_box.height - text_size,text_size, ACCENT_COLOR2);
                            }
                        }
                    }
                }
...
>>>>>>> fb57b5f (Input title text with overflow checks (backspace bug))
